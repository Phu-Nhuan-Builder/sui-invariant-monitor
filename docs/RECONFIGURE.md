...
```bash
cd ~
rm -rf first-movers-sprint-2026
```

### Bước 3: Clone repo mới

```bash
git clone https://github.com/Phu-Nhuan-Builder/sui-invariant-monitor.git first-movers-sprint-2026
cd first-movers-sprint-2026/backend
```
...
```ini
[program:sui-monitor]
command=/root/first-movers-sprint-2026/backend/target/release/sui-invariant-monitor
directory=/root/first-movers-sprint-2026/backend
user=root
autostart=true
autorestart=true
